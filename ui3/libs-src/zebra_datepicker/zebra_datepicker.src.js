...
/**
 * Modified by Brian Pearce to make the set_date method do what I want it to for my UI2 project.
 */
;(function(factory) {
    ...
    $.Zebra_DatePicker = function(element, options) {
         ...
         *  @return void
         */
        plugin.set_date = function(date, cell) {
            ...
            if ((dateObj = check_date(date)) && !is_disabled(dateObj.getFullYear(), dateObj.getMonth(), dateObj.getDate())) {
            	...
            	selected_year = dateObj.getFullYear();
            	selected_month = dateObj.getMonth();
            	manage_views();
            	var dayNum = dateObj.getDate().toString();
            	var cell = $(daypicker).find("td:contains(" + dayNum + ")").filter(function ()
            	{
            		return $(this).text() === dayNum && !$(this).hasClass("dp_not_in_month");
            	});
            	select_date(dateObj.getFullYear(), dateObj.getMonth(), dateObj.getDate(), 'days', cell);
        ...
        var generate_daypicker = function() {
            ...
            for (var i = 0; i < 7; i++)
            {
            	if($.isArray(plugin.settings.days_abbr) && undefined !== plugin.settings.days_abbr[(plugin.settings.first_day_of_week + i) % 7])
            	{
            		html += '<th>' + plugin.settings.days_abbr[(plugin.settings.first_day_of_week + i) % 7] + '</th>';
            	}
            	else
            	{
            		html += '<th>' + plugin.settings.days[(plugin.settings.first_day_of_week + i) % 7].substr(0, i == 4 ? 2 : 1) + '</th>';
            	}
            }
